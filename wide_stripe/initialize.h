...
struct ssd_info{ 
	...
	unsigned long page_num;                    //使用物理页条带组织RAID条带时，用来记录当前写的条带号。
	unsigned int band_channel;
#ifdef USE_WHITE_PARITY
	unsigned int change_current_t_flag ; 
	...
	struct recovery_operation *recovery_head;
	struct recovery_operation *recovery_tail;
#endif

	unsigned int write_sub_request;
	unsigned int read_sub_request;
...
#ifdef USE_EC
/*每个条带的信息，其中按照物理块进行条带组织，条带内的子条带页具有相同的信息*/
struct band_info{
...
#ifdef BROKEN_PAGE
	Status bad_page_flag;			  
#endif
};

/**/struct blk_map
{
	unsigned int pn;  //inidicate a stripe ppn;
	unsigned int state;  //inidicate a stripe sub page 
};
...
struct map_info{
	...
	int (*pbn)[BAND_WITDH];
#else
	int (*pbn)[PARITY_SIZE+1];
#endif
};

struct wear_map_info{
	struct wear_entry* wear_map_entry;      
};

struct wear_entry {
	unsigned int wear_state : 2;
};
...
struct request{
	...
	unsigned int* need_distr_flag;		//要为其分配一块内存空间，表示是否需要执行distribution函数，1表示需要执行，0表示不需要执行
...
struct sub_request{
	...
	int size;
#ifdef USE_WHITE_PARITY 
	int type ;//表示该子请求是垃圾回收子请求还是数据读写子请求。
#endif 
...
struct parameter_value{
	...
	int dynamic_allocation;		//记录动态分配的方式，0表示全动态分配，1表示channel定，chip、plane、die为动态分配
	int advanced_commands;		//是否使用高级命令，0表示不使用。利用二进制01分别表示random(00001)，copyback(00010)，two-plane-program(00100)，interleave(01000),two-plane-read(10000)是否使用，全部使用是11111，既31；
...
*********************************************************/
struct entry{                       
	int pn;                //物理号，既可以表示物理页号，也可以表示物理子页号，也可以表示物理块号
...
struct recovery_operation{
	...
	/*主动恢复需要通过buffer获取出错闪存页对应的lpn和state*/
	unsigned int broken_flag;
	int *broken_state; //数据丢失的页的状态
	unsigned int *broken_lpn;
#endif
	/*以下是被动恢复所需的字段*/
